...
export class LandingGame {
  ...
  private drawSelectionBox(shape: Shape) {
    ...
    if (shape.type === "rectangle") {
      ...
      const offsetX = length ? (dy / length) * borderOffset : 0;
      const offsetY = length ? -(dx / length) * borderOffset : 0;
      this.ctx.beginPath();
  ...
  private mouseDownHandler = (e: MouseEvent) => {
    ...
    if (this.selectedTool === "eraser") {
      this.shapes = this.shapes.filter(
        (shape) => !this.isPointInShape(shape, mouseX, mouseY)
      );
      this.clearCanvas();
      return;
    }
  ...
  private mouseMoveHandler = (e: MouseEvent) => {
    ...
    if (this.selectedTool === "select" && this.selectedShape) {
      const corner = this.isPointInHandle(this.selectedShape, mouseX, mouseY);
